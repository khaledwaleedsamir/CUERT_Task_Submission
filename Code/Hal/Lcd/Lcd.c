...
void H_Lcd_Goto(u8 Local_u8_Y,u8 Local_u8_X)
{
	if(Local_u8_Y == 1)
	{
		u16 position = 0x80 + Local_u8_X;
		H_Lcd_WriteCommand(position);
	}
	if(Local_u8_Y == 2)
	{
		u16 position = 0xC0 + Local_u8_X;
		H_Lcd_WriteCommand(position);
	}
}